module Solaris
  module Modbus
    class ADU
      ...
      def self.read_register(transaction, unit, address, quantity)
        new(
          TCPHeader.new(transaction: transaction, unit: unit),
          Request.new(function: 3, get: { address: address, quantity: quantity })
        )
      end
      ...
      def self.parse(binary_string)
        ...
        new(
          ...
          Response.read(binary_string[7..-1])
        )
      end

      def initialize(header, pdu)
        @header = header || TCPHeader.new
        @pdu = pdu || Request.new
      end
